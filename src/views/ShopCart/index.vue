      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cartInfo in cartInfoList" :key="cartInfo.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="cartInfo.skuNum" minnum="1" class="itxt" @change="handler('change',cartInfo,$event.target.value * 1)">
...
export default {
  ...
  methods: {
    ...
    // 改变购物车中商品购买数量
    handler: throttle(async function (type, cart, inputValue) {
      let disNum = 0
      switch (type) {
        case 'add':
          disNum = 1
        ...
        case 'change':
          if (isNaN(inputValue) || inputValue < 1) {
            ...
            disNum = parseInt(inputValue) - cart.skuNum
          }
          break
      }
      try {
        await this.$store.dispatch('detail/skucarInfo', { skuId: cart.skuId, skuNum: disNum })
        this.getData()
      } catch (error) { }
    }, 500),
    // 删除购物车单条数据
    ...
    async changeChecked(cart, e) {
      ...
      try {
        await this.$store.dispatch('shopcart/changeChecked', { skuId: cart.skuId, isChecked })
        this.getData()
      } catch (error) { }
    },
    // 删除所有选中商品
