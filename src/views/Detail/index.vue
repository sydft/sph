        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="addSkucar">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 类型选择（排他）
    changeActive(spuSaleAttrValue, attrs) {
      // console.log(spuSaleAttrValue, attr)
      attrs.forEach((attr) => {
        attr.isChecked = '0'
      })
    ...
    async addSkucar() {
      try {
        await this.$store.dispatch('detail/skucarInfo', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
  ...
  mounted() {
    // 获取商品信息
    this.$store.dispatch('detail/detailInfo', this.$route.params.skuId)
  }
}
</script>
